...
def test_single_event():
    ...
    # Summary
    if results:
        # Filter out Θ values outside the range [-8, -1]
        filtered_results = [r for r in results if -8 <= r.theta_estimated <= -1]
        
        if filtered_results:
            theta_values = [r.theta_estimated for r in filtered_results]
            ...
            print("FINAL RESULT:")
            print("="*70)
            print(f"  Stations processed: {len(filtered_results)}")
            print(f"  Average Θ: {theta_mean:.2f} ± {theta_std:.2f}")
            print(f"  Depth bin: {filtered_results[0].depth_bin}")
            print("="*70)
            ...
            print("\nNo valid Θ values within the range [-8, -1]!")
    else:
